function chequearBloqueoMes() {
    ...
    const btnTurnoNoche = document.getElementById("fab-turno-noche");

    // Cambiado: <= para bloquear también el mes actual
    const estaBloqueado = rolUsuario !== 2 && (
        anioSeleccionado < anioActual ||
        (anioSeleccionado === anioActual && mesSeleccionado <= mesActual)
    );

    if (overlay) overlay.style.display = estaBloqueado ? "block" : "none";
    ...
    if (btnAgregar) {
        ...
        btnAgregar.title = estaBloqueado ? "No se puede agregar bloques en el mes actual ni en meses anteriores" : "Agregar bloque";
    }
    if (btnTurnoNoche) {
        ...
        btnTurnoNoche.title = estaBloqueado ? "No se puede agregar turno noche en el mes actual ni en meses anteriores" : "Agregar turno noche";
    }
}
